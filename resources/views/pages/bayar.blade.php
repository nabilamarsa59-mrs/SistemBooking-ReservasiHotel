...
@section('content')
        ...
        <section id="section-reservasi" class="px-6 pt-8 md:px-10">
            @if ($reservasiAktif)
                @php
                    ...
                    $total  = $harga * $durasi;
                    $status = $reservasiAktif->status_reservasi ?? 'pending';
                @endphp
                <div class="border border-gray-400 bg-[#f2eee6] p-6 shadow-sm">
                    <div class="mb-5 flex items-center justify-between border-b border-gray-300 pb-4">
                        <div>
                            @if ($status === 'pending')
                                <h2 class="text-[24px] font-bold uppercase tracking-wide">Menunggu Konfirmasi</h2>
                                <p class="mt-1 text-[15px] text-[#6b7280]">Pemesanan Anda sedang menunggu verifikasi dari admin.</p>
                            @elseif ($status === 'aktif')
                        ...
                        @if ($status === 'pending')
                            <span class="rounded-full bg-yellow-400 px-5 py-2 text-[14px] font-semibold text-white">
                                Menunggu Konfirmasi
                            </span>
                        @elseif ($status === 'aktif')
                            <span class="rounded-full bg-[#7ea1ba] px-5 py-2 text-[14px] font-semibold text-white">
                                Aktif
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-6 lg:grid-cols-5">
                        <div class="lg:col-span-2">
                            ...
                            <p class="mt-1 text-[15px] text-[#6b7280]">
                                #RSV-{{ str_pad($reservasiAktif->id_reservasi, 3, '0', STR_PAD_LEFT) }}
                            </p>
                        </div>
                ...
                @forelse ($rooms as $room)
                    ...
                    <div class="flex h-full flex-col border border-gray-400 bg-[#f2eee6] p-4">
                        <div class="flex h-[220px] items-center justify-center overflow-hidden border border-gray-400 bg-[#f7f4ee]">
                            ...
                            @endif
                            <div class="hidden h-full w-full items-center justify-center text-[16px] text-[#243b53]">Gambar kamar</div>
                        </div>
                            ...
                            <div class="p-6 text-[#243b53]">
                                <div class="mb-4 flex items-start justify-between">
                                    ...
                                    <div class="text-right">
                                        <p class="text-[22px] font-bold">Rp {{ number_format($room->harga_kamar, 0, ',', '.') }}</p>
                                ...
                                <div class="mb-5 grid grid-cols-2 gap-2 text-[14px]">
                                    @foreach (explode(',', $room->fasilitas) as $fas)
                                        <div class="flex items-center gap-2 rounded-lg border border-gray-200 bg-white/60 px-3 py-2">
    ...
    <script>
        ...
        function cekKetersediaan() {
            ...
            infoTxt.textContent = `Menampilkan kamar tersedia: ${formatTanggal(ci)} – ${formatTanggal(co)} (${selectedDurasi} malam)`;
            infoEl.classList.remove('hidden'); infoEl.classList.add('flex');
            document.getElementById('btn-reset-tanggal').classList.remove('hidden');

            document.querySelectorAll('.total-harga-info').forEach(el => {
                el.textContent = `Total ${selectedDurasi} malam: ${formatRupiah(parseInt(el.dataset.price) * selectedDurasi)}`;
        ...
        function openRoomPopup(id) {
            ...
            if (el) { el.classList.remove('hidden'); el.classList.add('flex'); document.body.classList.add('modal-open'); }
        }

        function closeRoomPopup(id) {
            const el = document.getElementById('popup-' + id);
            if (el) { el.classList.add('hidden'); el.classList.remove('flex'); document.body.classList.remove('modal-open'); }
        }
